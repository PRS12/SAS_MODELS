...
select &&VAR&q.._P99 into :&&VAR&q.._P99 
from &TG_DATASET._MEAN ;
...
%end;
     ...
     %do q = 1 %to &cnt.;
		   ...
		   else if &&VAR&q..< &&&&&&VAR&q.._P99 then &&VAR&q.._FLAG="4";
           else &&VAR&q.._FLAG="5";
...
libname t '/sasdata3/ci/dkumar/temp_d/Triangle_Model';
libname dp '/sasdata3/ci/dkumar/temp_d/ICICI_KPI_CUST';

%MC_TG_CG_BALANCING(
TG_LIBNAME=%STR(dp)/* TG Dataset Libname */
,TG_DATASET=%STR(icici_intrnation) /* TG Dataset Name */
,BAL_VAR_LIST=%STR(Earn_MOB|Earn_pts_MOB|Earn_Recency) /* Variable list used for Balancing This can be more or less than 3 variable */
,BAL_TUNING=%STR(10) /* Tuning Parameter */
,CG_LIBNAME=%STR(dp) /* CG Dataset Libname */
,CG_DATASET=%STR(summ_act) /* CG Dataset Name */
,PRIMARY_KEY=%STR(CUSTOMER_ID) /* Unique Identification of Customers */
);
